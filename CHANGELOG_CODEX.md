...
- **Startup hang risk (high severity):** `LocalStorageService._mapToAvatar()` (`lib/core/services/local_storage_service.dart`) deserialized a stored avatar record with unguarded direct map access (`data['name']`, `DateTime.parse(data['createdAt'])`, etc.) and no error handling anywhere in the call chain above it — `getAvatar()` → `AvatarProvider.loadAvatar()` → `main.dart`'s fire-and-forget `_checkAppState()` (called from `initState()`, never awaited or wrapped there). A corrupt or partially-written avatar record — app killed mid-write, an old/incompatible schema missing a field — would throw during startup and **hang the splash screen forever**, with no error shown and no way forward for the player. Fixed at both layers: `getAvatar()` now catches deserialization failures and treats them as "no avatar" (the player can create a new one) rather than propagating; `_checkAppState()` also wraps the call defensively as defence-in-depth against any other unexpected startup failure reaching the same dead end.
- **Silent persistence-failure inconsistency (medium severity):** all six `AvatarProvider` mutators (`createAvatar`, `updateAvatarName`, `changeOutfit`, `unlockOutfit`, `unlockAccessory`, `addExperience`) assigned the new value to `_avatar` *before* awaiting `saveAvatar()`. If the save failed (storage error, disk full), the in-memory avatar already reflected a change that was never persisted — a level-up, outfit change, or unlock could appear to work, then silently vanish on the next app restart, with no indication anything had gone wrong in between. Reordered every mutator to build the new value, await the save, and only then commit it to `_avatar` — in-memory state can no longer drift ahead of what's actually on disk.
- Both bugs were in files with **zero prior test coverage** — every existing test used a stub `LocalStorageService` subclass that overrode `getAvatar()`/`saveAvatar()` directly, bypassing the real code entirely. Added `test/services/local_storage_service_test.dart` (5 tests, using a real temp-directory-backed Hive instance — the first test in the repo to exercise the actual deserialization path) and `test/services/avatar_provider_test.dart` (3 tests, using a fake storage service that can be told to fail on demand) to prove both fixes and guard against regression.
- Verified: `dart format` clean, `flutter analyze` 0 issues, **116/116 tests passing** (was 108; +5, +3), `flutter build web --release` succeeds.
- **Phase 1 of the `prompt.md` review (build/runtime/persistence defects) is now complete.** Continuing with Phase 2 onward (design-system token audit, systematic clipping/portrait pass, avatar renderer consolidation, question UI unification) in subsequent work.

## 2026-09-15 - Fixed Player Profile: it was reading a dead, always-empty data model

Root-caused and fixed the long-reported "Player Profile doesn't work" defect, as the first task of working through `prompt.md`'s full product/UX/engineering review (see `docs/PROMPT_MD_EXECUTION_TRACKER.md`, the working checklist for that larger effort — this session covered its Phase 0 baseline and the top of Phase 1; the rest is substantial remaining work, not claimed complete).

- **Root cause**: `lib/ui/screens/profile_stats_screen.dart` read from `StatsService`/`PlayerStats` (`lib/core/services/stats_service.dart`), a legacy data model that nothing else in the app writes to — no quest-completion path, `RewardTransactionService`, or game screen calls any of its mutating methods. `StatsService.getStats()` auto-creates and persists a fresh all-zero record the first time it's queried, so Player Profile always showed Level 1 / 0 XP / 0 stars / 0 activities / all zones but the starter locked, regardless of real play. Confirmed by grep: `StatsService` had exactly two referencing files in `lib/` — itself and the profile screen.
- **Fix**: rewired `ProfileStatsScreen` onto the app's real canonical sources — `AvatarProvider` (identity/level/XP), `SkillProvider` (zone/mastery progress, via the same `WorldMapViewModel.evaluateZoneState`/`calculateTotalStars`/`recommendedZoneIndex` the world map itself already uses, rather than a third reimplementation of the unlock rule), `ShopService` (star balance), `StreakService` (day streak), `QuestSessionHistoryService` (quests completed). Loading/empty/error/content states are now distinct — a hydration failure shows a retry action instead of silently rendering "No progress yet" over a real error. All 8 zones now appear (the old hard-coded list had drifted to 5, missing math_facts/science_explorers/creative_corner). Zone state is shown via label + icon + colour together, not colour alone.
- **Two duplicated/drifted models found and fixed along the way** (same failure class the brief calls out generally):
  - Companion-emoji mapping was independently duplicated in three places and had drifted apart: `world_map_adventure_bar.dart` recognised only 4 of the 16 real companion IDs (the other 12 showed a generic 🧭 in the world map HUD); `world_entry_screen.dart` recognised 12 of 16 (the other 4 incorrectly showed a bear 🐻). Consolidated into `lib/core/models/companion_catalog.dart`. Also fixed `avatar_creator_screen.dart`'s own list, which used placeholder letters ('D', 'T') for dragon/turtle instead of the real emoji (🐉/🐢) that exist for them.
  - The 8-zone catalog was hand-duplicated in `world_map_screen.dart`, `profile_stats_screen.dart` (stale, only 5 zones), `world_entry_screen.dart` and `fantasy_map.dart`. Extracted into `lib/core/models/zone_catalog.dart`; converged `world_map_screen.dart` onto it as a low-risk pure extraction (identical values, verified by the existing world-map test suite). The other two files' copies are noted as remaining debt, not converged in this change.
  - Decoupled `XpBar` (`lib/ui/widgets/xp_widgets.dart`) from the dead `PlayerStats` model — it now takes plain level/XP values, so it isn't tied to one specific stats model.
- **Two real bugs the rewrite introduced, caught by the verification loop before shipping** (worth noting since it demonstrates why the mandated format/analyze/test loop matters, not just "it compiles"): a zone-name + status-chip `Row` overflowed at 390px width (fixed with `Flexible`+ellipsis); a fixed-`childAspectRatio` stats `GridView` overflowed as soon as a label needed two lines and would have overflowed again at larger accessibility text scales regardless of tuning (replaced with a content-sized `Wrap`). Also found and fixed a latent Provider hazard unrelated to this screen specifically: `SkillProvider.initializeSkills()` calls `notifyListeners()` synchronously before its first `await`, which throws "setState() or markNeedsBuild() called during build" if a widget calls it directly from `initState()` — fixed by deferring via `Future.microtask`.
- **Test coverage**: `test/profile_stats_screen_test.dart` was rewritten, not extended — the previous version only pumped a `LocalStorageService` mock and never touched any of the providers the screen actually reads from, so it could pass while the screen displayed entirely fake data (exactly the "simplified mock that conceals hydration failures" pattern to watch for). The new version pumps the real provider graph (matching `test/world_map/world_map_regression_test.dart`'s established pattern) and seeds genuine non-zero state in `ShopService`/`StreakService`/`QuestSessionHistoryService`, asserting the screen shows those exact values.
- Verified: `dart format --set-exit-if-changed` clean, `flutter analyze` 0 issues, **108/108 tests passing** (was 107), `flutter build web --release` succeeds (pre-existing non-blocking `flutter_tts_web` WASM dry-run warning only).
- Not yet done: `docs/CURRENT_PRODUCT_AUDIT.md` write-up, screenshot baseline, avatar save/restore race-condition audit, and the remaining ~9 phases of `prompt.md` (design system audit, portrait/clipping systematic pass, avatar renderer consolidation, question UI unification, full visual asset programme). Tracked in `docs/PROMPT_MD_EXECUTION_TRACKER.md`.
