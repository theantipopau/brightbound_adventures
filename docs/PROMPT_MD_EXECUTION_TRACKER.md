...
- Not yet done: `docs/CURRENT_PRODUCT_AUDIT.md` formal write-up, screenshot baseline, and everything from Phase 2 onward (design-system token audit, systematic clipping/portrait pass, avatar renderer consolidation, question UI unification, full art-production programme). Continuing next session per the user's "work through it periodically" instruction.
